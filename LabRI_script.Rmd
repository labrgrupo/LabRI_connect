...
# NOTE 6: Provide the source of the dataset, specifying where it was obtained
#         from.
...
################################################################################
### 1.1. Checking and loading required packages ################################
################################################################################
...
if (length(missing_pkgs) > 0) {
  stop(
    paste0(
      "The report cannot be generated because the following required packages ",
      "are not available in the deployment environment: ",
      paste(missing_pkgs, collapse = ", "),
      ". Please install them in the deployment environment before rendering the report."
    )
  )
}
...
for (i in seq_len(n)) {
  row <- ((i - 1) %% nrow) * 2 + 1
  col <- ceiling(i / nrow)
  tabela[row, col] <- required_pkgs[i]
...
kableExtra::kbl(
  ...
  align = "c",
  col.names = rep("", ncol),
  caption = "<div style='text-align: justify; color: black;'><strong>Table 1.</strong> Availability and loading status of the required R packages in the execution environment.</div>"
) |>
  ...
  kableExtra::column_spec(
    ...
    color = ifelse(
      Table.1.data.frame == "TRUE", "#155724",
...
sample.size.select <- ifelse(
  is.na(sampling) == TRUE, sample.size,
...
lado <- ifelse(
  Double_sided == "x" | Double_sided == "X", "Two_sided",
  ifelse(Right_sided == "x" | Right_sided == "X", "Right_sided", "Two_sided")
)
...
MAG <- function(tabela.contagem,
  ...
  #### 1.6.6. Analysis of n_casas_decimais column ###############################
  ...
  #### 1.6.7. Map threshold to Freq_rel_acum ####################################
